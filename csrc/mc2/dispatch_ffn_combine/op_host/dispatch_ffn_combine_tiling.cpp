...
static ge::graphStatus DispatchFFNCombineTilingFuncImpl(gert::TilingContext *context)
{
    ...
    // workspace 尾部：align512 + 512B sidecar + 304×int32 profile（与 profile.h 一致）
    constexpr uint64_t kDffcSidecarGmBytes = 512U;
    constexpr uint64_t kDffcProfileGmBytes = 304U * sizeof(int32_t);
    uint64_t sidecarOff = (userWorkspace + 511ULL) & ~511ULL;
    tilingData->cocTiling.profileGmOffset = sidecarOff + kDffcSidecarGmBytes;
    userWorkspace = tilingData->cocTiling.profileGmOffset + kDffcProfileGmBytes;
    userWorkspace = (userWorkspace + 511ULL) & ~511ULL;
#endif
