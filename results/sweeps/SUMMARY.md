...
> 环境：EP=8，Ascend 910，`dispatch_ffn_combine` + profile v3  
> 数据目录：`results/sweeps/by_{numtokens,hidden,imhidden,topk}/`  
> 趋势图：各轴 `summary_bandwidth.png`
...
| 轴 | 固定参数 | 遍历 | 有效点数 | 趋势图 |
|----|----------|------|----------|--------|
| numtokens | K=4096, N=4096, topk=8 | M=1K~16K | 5（1 点 dispatch 异常） | `by_numtokens/summary_bandwidth.png` |
| hidden | M=8192, topk=8, N=K | K=2K~8K | **8**（K≥8064 aicore 异常，**未达 HBM OOM**） | `by_hidden/summary_bandwidth.png` |
| imhidden | M=8192, K=4096, topk=8 | imhidden=1K~3K | 3（imhidden=4K 失败） | `by_imhidden/summary_bandwidth.png` |
| topk | M=8192, K=4096, N=4096 | topk=4,8,16 | 3 | `by_topk/summary_bandwidth.png` |
...
验收：`tools/dffc_profile/validate_profile.py --strict`；`dur=0` 在 `gen_timeline.py` 以虚线框标注。
