...
from tools.dffc_profile.cases import DffcCase
from tools.dffc_profile.profile_constants import DFFC_PROFILE_VERSION
...
SWEEP_AXES = ("by_hidden", "by_imhidden", "by_numtokens", "by_topk")
...
def _run_one_case_job(
    *,
    ...
    cmd = [
        sys.executable,
        str(_TOOLS_DIR / "run_one_case.py"),
        "--m", str(case.m),
        "--k", str(case.k),
...
def _refresh_manifest_bandwidth(axis_dir: Path) -> None:
    ...
    for entry in manifest.get("cases", []):
        ...
        job = bw.get("job") or {}
        pr_d = bw.get("per_rank_bw_dispatch_gbps") or {}
        pr_c = bw.get("per_rank_bw_combine_gbps") or {}
        entry["bandwidth"] = {
            "dispatch_gbps_job": job.get("bw_dispatch_gbps"),
            "combine_gbps_job": job.get("bw_combine_gbps"),
            "dispatch_gbps_per_rank_median": pr_d.get("median"),
            "combine_gbps_per_rank_median": pr_c.get("median"),
        }
...
def _regen_axis_summaries(sweeps_root: Path) -> None:
    for axis in SWEEP_AXES:
        axis_dir = sweeps_root / axis
        if not axis_dir.is_dir():
            continue
        subprocess.run(
            [
                sys.executable,
                str(_TOOLS_DIR / "plot_sweep_timelines.py"),
                "--sweep-dir",
                str(axis_dir),
            ],
            check=False,
        ...
        subprocess.run(
            [
                sys.executable,
                str(_TOOLS_DIR / "plot_sweep_bandwidth.py"),
                "--sweep-dir",
                str(axis_dir),
            ],
            check=False,
...
def main() -> int:
    ...
    roots = [Path(p) for p in args.root] if args.root else [
        _REPO_ROOT / "results/sweeps",
        _REPO_ROOT / "results/try_data",
    ]
    ...
    if not args.skip_summaries:
        sweeps_root = _REPO_ROOT / "results/sweeps"
