...
from tools.dffc_profile.parse_profile import load_rank_profiles, aggregate
...
from tools.dffc_profile.profile_utils import u32
from tools.dffc_profile.timeline_backup import read_aggregation_marker
...
def validate_indir(
    ...
    *,
    ...
    if g2["timeline_duration_us"] >= 1 and sw["timeline_duration_us"] >= 1 and g2s < sw["timeline_start_us"] - 1:
        issues.append(
            f"job: gmm2.start ({g2s:.0f}) < swiglu.start ({sw['timeline_start_us']:.0f})"
        )
    if g2s < g1s - 1:
        issues.append(f"job: gmm2.start ({g2s:.0f}) < gmm1.start ({g1s:.0f})")
    if _is_combine_v1(case_params):
        if comb["timeline_duration_us"] >= 1 and g2["timeline_duration_us"] >= 1 and cs < g2e - 50:
            issues.append(f"job: combine.start ({cs:.0f}) < gmm2.end ({g2e:.0f}) [V1]")
    elif comb["timeline_duration_us"] >= 1 and g2["timeline_duration_us"] >= 1 and cs < g2s - 50:
        issues.append(f"job: combine.start ({cs:.0f}) < gmm2.start ({g2s:.0f}) [V2]")
    ...
    for p in profiles:
        ...
        if ph["gmm1"]["duration_us"] >= 1 and ph["gmm2"]["duration_us"] >= 1:
            if ph["gmm2"]["begin_us"] < ph["gmm1"]["begin_us"]:
                issues.append(f"rank{rank}: gmm2.begin < gmm1.begin")
        if ph["swiglu"]["duration_us"] >= 1 and ph["gmm2"]["duration_us"] >= 1:
            if ph["gmm2"]["begin_us"] < ph["swiglu"]["begin_us"]:
                issues.append(f"rank{rank}: gmm2.begin < swiglu.begin")
